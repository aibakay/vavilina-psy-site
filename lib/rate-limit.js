...
//
// Границы применимости: общий каталог = общая файловая система. Работает, пока
// процессы стоят на одном хосте (PM2/cluster, несколько контейнеров с общим
// томом в AUTH_RATE_LIMIT_DIR). Реплики на разных хостах друг друга не видят —
// как и serverless, где у каждого инстанса свой эфемерный /tmp.
//
// Если хостов станет больше одного, счётчик надо унести в Redis: заменить
// fileStore на INCR по ключу `auth:<окно>:<хэш IP>` c pExpire на длину окна
// (схема ключей та же, TTL снимает нужду в подметании), `consume` при этом
// станет асинхронной. Готовая альтернатива — express-rate-limit с адаптером
// rate-limit-redis, но она тянет зависимости, а Redis в инфраструктуре пока
// нет, поэтому ради одного счётчика на одном VPS смысла в ней нет.
...
// не чаще раза в окно. Таймер пришлось бы держать живым и unref'ить ради работы,
// которой в простое всё равно нет — попыток входа не было, чистить нечего.
function sweep(now) {
...
/**
 * IP клиента. За Express с `trust proxy` адрес уже разобран в `req.ip` — это
 * рабочий путь. Остальные ветки на случай, если обработчик повесят на голый
 * http-сервер без Express: без них у всех клиентов оказался бы один ключ
 * `unknown`, то есть один перебор запирал бы вход всем сразу.
 */
